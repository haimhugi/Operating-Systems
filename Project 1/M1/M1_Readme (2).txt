...
Part 2 - Check student homework 
	...
	Pseudo code algorithm:
		...
		readdir(students) V
		for each students: 
			execl(gcc student_name.c -o main.out) V
			open(test_input.qa, O_RDONLY) V
			open(program_output.txt, O_WRONLY | O_CREAT | O_TRUNC ) V
			dup2(test_input.qa , keyboard) V
			dup2(program_output.txt , screen) V
			exccl(main.out) V
			open(results.csv, O_WRONLY | O_CREAT | O_APPEND) V
			dup2(results.csv , screen) V
